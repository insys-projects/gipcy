...
IPC_handle IPC_openLibraryEx(const IPC_str *baseName, unsigned param)
{
	...
	wchar_t* pchar = wcsstr(libname, _BRDC(".dll"));
	if(pchar != NULL)
		*pchar = 0;
	...
	{
		wcscat(libname, _BRDC("64"));
		wcscat(libname0, _BRDC("-64"));
	}
            h->ipc_descr = IPC_openLibrary( libname, 0 );
            if( !h->ipc_descr )
	{
                    h->ipc_descr = IPC_openLibrary( libname0, 0 );
	}
}
#else
IPC_handle IPC_openLibraryEx(const IPC_str *baseName, unsigned param)
