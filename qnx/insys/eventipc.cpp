...
IPC_handle IPC_createEvent(const IPC_str *name, bool manual, bool value)
{
    ...
    int res = devctl(fd, IOCTL_IPC_EVENT_OPEN, &ipc_param, sizeof(ipc_param), NULL);
    if(res < 0) {
...
int IPC_waitEvent(const  IPC_handle handle, int timeout)
{
    ...
    int res = devctl(fd, IOCTL_IPC_EVENT_LOCK, &ipc_param, sizeof(ipc_param), NULL);
    if(res < 0) {
...
int IPC_setEvent(const  IPC_handle handle)
{
    ...
    int res = devctl(fd, IOCTL_IPC_EVENT_UNLOCK, &ipc_param, sizeof(ipc_param), NULL);
    if(res < 0) {
...
int IPC_resetEvent(const  IPC_handle handle)
{
    ...
    int res = devctl(fd, IOCTL_IPC_EVENT_RESET, &ipc_param, sizeof(ipc_param), NULL);
    if(res < 0) {
...
int IPC_deleteEvent(IPC_handle handle)
{
    ...
    int res = devctl(fd, IOCTL_IPC_EVENT_CLOSE, &ipc_param, sizeof(ipc_param), NULL);
    if(res < 0) {
