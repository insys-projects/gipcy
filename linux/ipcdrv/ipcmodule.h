...
union ipc {
    int          ipc_sem;           //!< Семафор
    int          ipc_mutex;         //!< Мьютекс
    int          ipc_event;         //!< Событие
};

//-----------------------------------------------------------------------------

//! Структура описывает внутренний тип struct ipc_sem_t используемый библиотекой IPC
/*!
    struct ipc_sem_t - это внутренний тип данных, используемый библиотекой IPC,
